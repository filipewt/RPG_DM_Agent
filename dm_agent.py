...
class DMAgent:
    ...
    def __init__(self, openai_api_key: str = None):
        ...
        self.experience_system = ExperienceSystem(
            self.character_manager, self.rule_engine
        )
        ...
        if openai_api_key:
            ...
            if api_key:
                ...
                self.client = None
                self.logging_system.logger.warning(
                    "No OpenAI API key provided. LLM features will be limited."
                )
    ...
    def start_new_adventure(
        ...
        try:
            ...
            self.current_session_id = self.logging_system.start_session(
                character_name, "adventure"
            )
            ...
            self.current_character = self.character_manager.create_character(
                character_name,
                "Vampire_The_Masquerade_Character_Sheet",
                custom_attributes,
            )
            ...
            journal_path = self.journal_manager.create_journal(
            ...
            opening_narrative = self._generate_opening_narrative(
    ...
    def continue_adventure(self, character_name: str) -> bool:
        ...
        try:
            ...
            # Load character
            self.current_character = self.character_manager.load_character(
                character_name
            )
    ...
    def process_player_input(self, player_input: str) -> str:
        ...
        try:
            if not self.current_character:
                return (
                    "No active character. Please start or continue an adventure first."
                )
    ...
    def _generate_fallback_response(self, player_input: str) -> str:
        ...
        if any(word in input_lower for word in ["attack", "fight", "combat"]):
            ...
            word in input_lower for word in ["look", "examine", "search", "investigate"]
        ):
    ...
    def _perform_hidden_dice_roll(
        self, dice_pool: int, difficulty: int, action_type: str
    ) -> str:
    ...
    def _identify_skill_check(self, player_input: str) -> dict:
        ...
        if any(
            word in input_lower
            for word in ["attack", "fight", "combat", "strike", "hit"]
        ):
            if self.current_character:
                attrs = self.current_character.get("attributes", {})
                physical = attrs.get("physical", {})
        ...
        # Perception/Investigation actions
            ...
            word in input_lower
            for word in ["look", "examine", "search", "investigate", "observe", "watch"]
        ):
            if self.current_character:
                attrs = self.current_character.get("attributes", {})
                mental = attrs.get("mental", {})
        ...
        # Social actions
            ...
            word in input_lower
            for word in ["talk", "speak", "say", "ask", "persuade", "convince", "charm"]
        ):
            if self.current_character:
                attrs = self.current_character.get("attributes", {})
                social = attrs.get("social", {})
        ...
        # Movement/Stealth actions
            ...
            word in input_lower
            for word in ["move", "go", "walk", "run", "sneak", "hide", "stealth"]
        ):
            if self.current_character:
                attrs = self.current_character.get("attributes", {})
                physical = attrs.get("physical", {})
    ...
    def _create_test_description(self, test_info: dict) -> str:
        ...
        action_desc = action_descriptions.get(
            test_info["action_type"], "You attempt your action."
        )
    ...
    def _build_llm_context(self) -> str:
        ...
        if self.current_character:
            context_parts.append(
                f"Character: {self.current_character.get('name', 'Unknown')}"
            )
            context_parts.append(f"Level: {self.current_character.get('level', 1)}")
            context_parts.append(
                f"Class: {self.current_character.get('class', 'Unknown')}"
            )
...
**Remember:** You are the storyteller, not just a rule enforcer. Make the story come alive!"""
    ...
    def _generate_opening_narrative(
        self, character_name: str, character_class: str = None
    ) -> str:
...
What would you like to do first?"""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a master storyteller writing opening scenes for Vampire: The Masquerade RPG. Write in second person, be atmospheric and engaging.",
                    },
    ...
    def roll_dice_pool(
        ...
        try:
            result = self.dice_roller.roll_dice_pool(
                attribute, skill, difficulty, hunger
            )

            # Log the roll
            character_name = (
                self.current_character.get("name") if self.current_character else None
            )
    ...
    def roll_dice(self, expression: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Log the roll
            self.logging_system.log_dice_roll(
                result,
                self.current_character.get("name") if self.current_character else None,
            )
    ...
    def award_experience(self, amount: int, source: str = None) -> bool:
        ...
        try:
            ...
            success = self.experience_system.award_experience(
                self.current_character, amount, source
            )
            ...
            if success:
                ...
                # Check for level up
                if self.experience_system.check_level_up_available(
    ...
    def get_character_info(self) -> dict[str, Any] | None:
