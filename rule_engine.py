...
class RuleEngine:
    ...
    def resolve_combat_attack(
        ...
        try:
            ...
            if hit:
                if weapon:
                    damage_roll = self.dice_roller.roll_damage(
                        weapon.get("damage", "1d4")
                    )
                    damage_bonus = (
                        ability_modifier if weapon.get("type") != "ranged" else 0
                    )
    ...
    def get_character_creation_info(self) -> dict[str, Any]:
        ...
        """
        try:
            creation_rules = self.get_rule(
                "Vampire_The_Masquerade_Character_Creation_rules", "character_creation"
            )
        ...
        try:
            ...
            if not creation_rules:
                validation_result["warnings"].append(
                    "Character creation rules not found"
                )
    ...
    def _validate_attributes(
        self, character: dict[str, Any], rules: dict[str, Any]
    ) -> dict[str, list[str]]:
                ...
                for attr in attrs:
                    ...
                    if attr_lower not in category_attrs:
                        result["warnings"].append(
                            f"Missing {attr} in {category} attributes"
                        )
                    elif (
                        not isinstance(category_attrs[attr_lower], int)
                        or category_attrs[attr_lower] < 1
                        or category_attrs[attr_lower] > 5
                    ):
                        result["errors"].append(
                            f"Invalid {attr} value: {category_attrs[attr_lower]} (must be 1-5)"
                        )
            ...
            for category, skill_list in skill_categories.items():
                ...
                for skill in skill_list:
                    ...
                    if skill_lower not in category_skills:
                        result["warnings"].append(
                            f"Missing {skill} in {category} skills"
                        )
                    elif (
                        not isinstance(category_skills[skill_lower], int)
                        or category_skills[skill_lower] < 0
                        or category_skills[skill_lower] > 5
                    ):
                        result["errors"].append(
                            f"Invalid {skill} value: {category_skills[skill_lower]} (must be 0-5)"
                        )
