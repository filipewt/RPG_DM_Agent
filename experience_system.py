...
class ExperienceSystem:
    ...
    def award_experience(self, character: Dict[str, Any], amount: int, 
        ...
        try:
            ...
            if success:
                ...
                if source:
                    self.logger.info(f"Awarded {amount} XP to {character.get('name', 'character')} from {source}")
                else:
                    self.logger.info(f"Awarded {amount} XP to {character.get('name', 'character')}")
                
                # Check for level up
                if self.rule_engine.check_level_up(character):
                    self.logger.info(f"{character.get('name', 'character')} is ready to level up!")
    ...
    def get_level_up_options(self, character: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            level_up_rules = self.rule_engine.get_rule('character_creation', f'leveling.{class_name}')
                ...
                for ability in level_up_rules.get('abilities', []):
                    if ability.get('level', 1) <= current_level:
                        options.append({
                            ...
                            'description': ability.get('description', 'A new ability'),
    ...
    def apply_level_up(self, character: Dict[str, Any], option: Dict[str, Any], 
    ...
    def _apply_attribute_improvement(self, character: Dict[str, Any], 
                                   details: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Increased {attribute_name} from {current_value} to {new_value}")
    ...
    def _apply_skill_improvement(self, character: Dict[str, Any], 
                               details: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Increased {skill_name} from {current_value} to {new_value}")
    ...
    def _apply_ability_gain(self, character: Dict[str, Any], option: Dict[str, Any], 
                           details: Dict[str, Any]) -> bool:
        """Apply ability gain"""
        try:
            ...
            # Update timestamp
            character['last_updated'] = self.character_manager.datetime.now().isoformat()
    ...
    def complete_level_up(self, character: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                ...
                current_hp = character.get('current_hit_points', character.get('max_hit_points', new_hp))
                if current_hp == character.get('max_hit_points', new_hp):
                    self.character_manager.update_character_attribute(
                        character, 'current_hit_points', new_hp
                    )
                
                self.logger.info(f"{character.get('name', 'character')} leveled up to level {new_level}!")
                return True
    ...
    def get_experience_progress(self, character: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get XP requirements
            current_level_xp = self.rule_engine.get_experience_requirements(current_level)
            next_level_xp = self.rule_engine.get_experience_requirements(current_level + 1)
            
            # Calculate progress
            xp_needed = next_level_xp - current_level_xp
            xp_gained = current_xp - current_level_xp
            progress_percentage = (xp_gained / xp_needed * 100) if xp_needed > 0 else 100
    ...
    def calculate_encounter_xp(self, encounter_type: str, difficulty: str = "medium", 
                             participant_level: int = 1) -> int:
        """
